...
* 2-dense vertical histograms at 1/4 vertical resolution (2-dense at 1/2 is
  already possible using quadrants).
* 4-dense horizontal histograms at the cost of going from 1/8 to 1/2 horizontal
  resolution (2-dense at 1/2 is already possible using quadrants).
* Somewhat more flexible borders, such as blocks, wave patterns, chevrons,
  fancy corners.
* Animated borders where the pattern slowly changes or progresses.

Amazing! However, they are missing from my terminal font. Questions:

* How hard is it to patch these characters into my terminal font?
* Are they in unscii? (If not, unscii braille already has this format).
* How widely available are them in other console fonts?
